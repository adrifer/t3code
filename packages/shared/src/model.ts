...
export function normalizeCopilotModelOptionsWithCapabilities(
  ...
  const value = getProviderOptionCurrentValue(reasoning);
  return typeof value === "string" ? [{ id: "reasoningEffort", value }] : undefined;
}

export function resolveCopilotApiModelId(model: string): string {
  return COPILOT_API_MODEL_IDS[model] ?? model;
}

export function resolveApiModelId(modelSelection: ModelSelection): string {
  return resolveCopilotApiModelId(modelSelection.model);
}
