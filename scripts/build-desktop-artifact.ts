...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    // WSL is Windows-only. That backend launches plain `wsl.exe -- node`, which
    // cannot read inside an asar archive, so the Windows artifact must unpack the
    // server bundle and its production dependencies. Other platforms keep the
    // smaller default unpack set to avoid exhausting file descriptors while
    // electron-builder packages thousands of pnpm dependency files.
    asarUnpack: [
      ...DESKTOP_ASAR_UNPACK,
      ...(platform === "win" ? ["apps/server/dist/**", "**/node_modules/**"] : []),
    ],
  };
