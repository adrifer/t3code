...
export const FALLBACK_COPILOT_MODEL_CATALOG: ReadonlyArray<CopilotModelCatalogEntry> = [
  ...
  { slug: "claude-opus-4-8-fast", name: "Claude Opus 4.8 (fast mode)" },
  ...
  { slug: "mai-code-1-flash", name: "MAI-Code-1-Flash" },
...
const KNOWN_COPILOT_MODEL_NAME_PATTERNS = [
  /Claude\s+(?:Sonnet|Opus|Haiku|Fable)\s+\d+(?:\.\d+)?(?:\s*\([^)]*\))*/gi,
  /GPT-[A-Za-z0-9.-]+(?:\s+(?:mini|nano|Sol|Terra|Luna|Codex))?/gi,
  /Gemini\s+\d+(?:\.\d+)?\s+(?:Pro|Flash)/gi,
  /MAI-Code-\d+-Flash/gi,
  /Raptor\s+mini/gi,
...
function slugFromNormalizedCopilotModelDisplayName(normalized: string): string | null {
  ...
  if (/^gemini\s+/i.test(normalized)) {
    return normalized.toLowerCase().replace(/\s+/g, "-");
  }
  if (/^mai-code-/i.test(normalized)) {
    return normalized.toLowerCase();
  }
  if (/^raptor\s+mini$/i.test(normalized)) {
    return "raptor-mini";
  }
  if (/^kimi\s+/i.test(normalized)) {
    return normalized.toLowerCase().replace(/\s+/g, "-");
  }
  if (/^grok\s+/i.test(normalized)) {
    return normalized.toLowerCase().replace(/\s+/g, "-");
  }

  const claudeMatch = /^Claude\s+(Sonnet|Opus|Haiku|Fable)\s+(\d+(?:\.\d+)?)(?<suffix>.*)$/i.exec(
    normalized,
  );
  if (!claudeMatch) {
    return null;
