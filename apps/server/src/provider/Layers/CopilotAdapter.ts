...
interface CopilotSessionContext {
  ...
  currentReasoningEffort: CopilotReasoningEffort | undefined;
...
export function mapInteractionModeToSessionMode(
          ...
          try {
            return providerSessionId
              ? await client.resumeSession(
                  providerSessionId,
                  sessionConfig satisfies ResumeSessionConfig,
                )
  ...
  const sendTurn: ProviderAdapterShape<ProviderAdapterError>["sendTurn"] = (input) =>
    Effect.gen(function* () {
      ...
      if (context.activeTurn) {
        if (
          (context.session.model && modelSelection.model !== context.session.model) ||
          reasoningEffort !== context.currentReasoningEffort
        ) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "sendTurn",
            issue: `Thread '${input.threadId}' already has an active turn. Wait for it to finish before switching model settings.`,
          });
        }
      ...
      if (
        context.session.model !== modelSelection.model ||
        context.currentReasoningEffort !== reasoningEffort
      ) {
        const setModelOptions = reasoningEffort ? { reasoningEffort } : undefined;
        yield* Effect.tryPromise({
