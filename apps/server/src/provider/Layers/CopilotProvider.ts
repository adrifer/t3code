...
const SDK_STOP_TIMEOUT_MS = 1_000;
...
function parseCopilotAuthStatusFromEnvironment(
  environment: NodeJS.ProcessEnv,
): CopilotAuthProbe | null {
  if (nonEmptyTrimmed(environment.COPILOT_PROVIDER_BASE_URL)) {
    return {
      status: "ready",
      auth: {
        status: "unknown",
  ...
  for (const variable of COPILOT_AUTH_TOKEN_ENV_VARS) {
    if (typeof environment[variable] === "string" && environment[variable]!.trim().length > 0) {
      return {
        status: "ready",
        auth: {
...
async function stopCopilotClient(client: CopilotClient): Promise<void> {
  try {
    const cleanupResult = await Effect.runPromise(
      Effect.promise(() => client.stop()).pipe(
        Effect.timeoutOption(Duration.millis(SDK_STOP_TIMEOUT_MS)),
      ),
    );
    if (Option.isNone(cleanupResult) || cleanupResult.value.length > 0) {
...
export const checkCopilotProviderStatus = (
  ...
  Effect.gen(function* () {
    ...
    const probe = yield* probeCopilotSdkStatus(copilotSettings, environment).pipe(
      Effect.catch((cause) => {
